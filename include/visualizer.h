...
private:
    ...
    void setVectorToArray(vtkDoubleArray* array, vtkIdType cellId, const Vector3D& vec);
    double computeVectorNorm(const Vector3D& vec, NormType normType);  // Always uses Euclidean norm for individual vectors
...
public:
    ...
    void computeGradientDifference(const std::string& outputArrayName = "GradientDifference");
    ...
    // L1: sum of all cell errors, L2: sqrt(sum of squares), L∞: maximum cell error
    double computeErrorNorm(NormType normType);
    
    // Computes Euclidean norm of error vector per cell and stores in array
    void computeErrorNormPerCell(NormType normType, const std::string& outputArrayName = "ErrorNorm");
